...
public final class CharacterJournal implements AutoCloseable {
    ...
    CharacterJournal(Path path, Store store) {
        ...
        if (Files.exists(path)) {
            try (Reader reader = Files.newBufferedReader(path, StandardCharsets.UTF_8)) {
                ...
                if (loaded == null || loaded.version != 1 || loaded.characters == null || loaded.accounts == null)
    ...
    public synchronized String observe(Entity player, int characterId) {
        ...
        // Death marks are explicit, reversible annotations; later packets cannot erase them.
        if (record.dead) return account;
        ...
        if (seasonal != null) record.seasonal = seasonal == 1;
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (total != null && bonus != null) record.stats[i] = Math.max(0, total - bonus);
        }
        for (int i = 0; i < 28; i++) {
            int stat = i < 12 ? 8 + i : 131 + i - 12;
            StatData item = player.stat.get(stat);
            if (item != null) record.equipment[i] = item.statValue;
        }
        long now = System.currentTimeMillis();
        if (!sameObservation(before, record) || now - record.lastSeen >= 30_000) {
            record.lastSeen = now; changed();
        }
        return account;
    }
    ...
    public synchronized void mergeRoster(String account, List<RealmCharacter> chars) {
        ...
        for (RealmCharacter c : chars) {
            ...
            if (r.dead) continue;
            r.classId = c.classNum; r.className = c.classString; r.level = c.level;
            r.name = account(account).name; r.skin = c.skin; r.fame = c.fame; r.seasonal = c.seasonal;
            r.created = c.date; r.source = "Character list + capture";
            int[] values = {c.hp, c.mp, c.atk, c.def, c.spd, c.dex, c.vit, c.wis};
            for (int i = 0; i < 8; i++) if ((c.capturedStatMask & (1 << i)) != 0) r.stats[i] = values[i];
            if (c.equipment != null) for (int i = 0; i < Math.min(28, c.equipment.length); i++) r.equipment[i] = c.equipment[i];
