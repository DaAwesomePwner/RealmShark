...
public final class LootArchiveAdapter implements ArchiveAdapter<Row,Facets,Sort> {
    ...
    public List<ReadSnapshot.Source> sources(SessionStore store,ArchiveQuery<Facets,Sort> q){return Collections.singletonList(new ReadSnapshot.Source(q.resolvedScope(store),"loot"));}
    ...
    public Map<String,String> dependencies(){Map<String,String> values=new LinkedHashMap<>();values.put("loot-projection","v1; saved item classifications; summaries limited to 25000 distinct keys; observed, not owned");if(definitions!=null)values.put("asset-generation",definitions.description());return values;}
    ...
    public void scan(ReadSnapshot pin,ArchiveQuery<Facets,Sort> q,Sink<Row> sink,Cancellation cancel)throws IOException{
        ...
        Map<String,Long> choices=new TreeMap<>();long[] totals=new long[4];
        pin.read("loot",LootDashboard.Drop.class,source->{
            ...
            if(!q.bounds().contains(LootQuery.time(d.time),LootQuery.time(d.time))||!f.location(d.bag,dungeon)||f.view==View.WHITES&&!white(d.bag))return;
            ...
            for(int ordinal=0;ordinal<d.items.size();ordinal++){
                ...
                Row row=Row.item(source.ref.session,d,i,dungeon);
                if(view==View.OCCURRENCES)sink.accept(source.child("item-"+ordinal,row));
