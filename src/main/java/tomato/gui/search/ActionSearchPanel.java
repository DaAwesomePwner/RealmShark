...
public final class ActionSearchPanel extends JPanel {
    ...
    public ActionSearchPanel(ActionRegistry registry,Runnable beforeOpen){
        ...
        JScrollPane resultScroll=new JScrollPane(results),detailScroll=new JScrollPane(details);
        resultScroll.setMinimumSize(new Dimension(0,60));detailScroll.setMinimumSize(new Dimension(0,135));
        resultScroll.setPreferredSize(new Dimension(450,180));detailScroll.setPreferredSize(new Dimension(450,170));
        JSplitPane split=new JSplitPane(JSplitPane.VERTICAL_SPLIT,resultScroll,detailScroll);split.setResizeWeight(.5);
