...
- **UX-06:** the full shell/history opens before asset setup. Choose assets and
  ...
  no source disclose unverified freshness. Capture is explicit-action only, with
  typed waiting/receiving/stopping/Npcap/error states. Missing Npcap no longer
...
- Startup no longer honors the old auto-start sniffer preference. Optional sprite
  failures use neutral images; core readiness validates definition catalogs.
