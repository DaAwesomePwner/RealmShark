...
Runs with no recorded loot remain in the denominator. Loot is joined to the visit ID captured with the bag, using the shared canonical dungeon name. Missing durations or unassigned drops produce **—** for unavailable rates. Legacy run-only imports have no measured loot coverage and are excluded from loot-rate denominators. These metrics describe captured drops, not pickups, guaranteed drop chances, or a server-wide total.
