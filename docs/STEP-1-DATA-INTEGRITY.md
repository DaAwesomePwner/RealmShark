...
`.github/workflows/validate.yml` runs the full regular test suite, JAR build, and help smoke check on Windows with JDK 17 for pull requests and pushes to `main`. GitHub Actions omits `--offline` so a fresh runner can obtain dependencies, uses `build/ci`, and uploads test reports and the runnable JAR. Hosted results are tracked on the pull request.

The hosted UI job prepares a supported primary-display mode with room for a 1240×800 logical window at native scale. `scripts/Set-CiDisplay.ps1` is restricted to Windows GitHub Actions runners; it checks native mode-change results and runs `CheckUiDisplay.java` at 100% to verify the actual window size and AWT transform. An insufficient runner desktop fails explicitly instead of silently substituting compact layouts for desktop checks. The popup keyboard test also waits for real Swing focus transfer before activating Enter.

The hosted Hyper-V video adapter was measured at 1024×768 by default, with supported modes capped at 1920×1080. That cannot accommodate the complete desktop fixture at 200%. The 150% and 200% results above are local validation, not hosted CI results; run the documented scaled tasks locally for UI/font changes. Hosted CI executes every regular test without exclusions.
